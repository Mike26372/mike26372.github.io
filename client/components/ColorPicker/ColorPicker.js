...
class ColorPicker extends Component {
  ...
  render() {
    ...
    let labelStyle = `${styles.colorLabel} ${
    ...
    return (
      <div className={colorContainer} style={togglePosition}>
        <div>
          ...
          <label htmlFor="base" className={labelStyle} />
          <span className={undoContainer} onClick={this.handleColor}>
            <i className={undoStyle} aria-hidden="true" />
          </span>
        </div>
        <div className={styles.text}>Fun with colors!</div>
        <div onClick={this.handleClick}>
          <i className={caretStyle} aria-hidden="true" style={caretRotation} />
        </div>
      </div>
    );
  }
