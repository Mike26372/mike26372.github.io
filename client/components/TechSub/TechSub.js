...
const TechSub = (props) => {
  ...
  let row5Columns = `${PureGrid['pure-u-1']} ${PureGrid['pure-u-md-1-5']} ${styles.row}`;
  ...
  return (
    <div className={row5Columns}>
      ...
      <div className={ mainStyle }>
       {mainSkills.map(skill => <div>{skill}</div>)}
      </div>    
      <div className={ subStyle }>
        {subSkills.map(sub => <div className={styles.subItem}>{sub}</div>)}
      </div>    
    </div>
  )
}
